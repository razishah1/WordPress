 ...
 * @since 1.0.0
 */
if (!class_exists('portfolio_kit_About')) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class portfolio_kit_About
	{
		/**
		 ...
		 *
		 * @var portfolio_kit_About $instance The  TI_About_Page instance.
		 */
		...
		{
			if (!isset(self::$instance) && !(self::$instance instanceof portfolio_kit_About)) {
				self::$instance = new portfolio_kit_About;
		...
		public function ti_about_page_render()
		{
			...
			if (!empty($welcome_title) || !empty($welcome_content) || !empty($this->tabs)) {
				...
				echo '<div class="wrap about-wrap portfolio-kit-about-wrap">';
				...
				echo '<a href="' . esc_url('https://wpthemespace.com/product/portfolio-kit-pro/') . '" target="_blank" class="wp-badge epsilon-welcome-logo"></a>';
...
?>
			<div class="feature-section portfolio-kit-section portfolio-kit-section-getting-started three-col">
				<?php foreach ($content as $item) : ?>
					<?php $this->render_grid_item($item); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .portfolio-kit-section -->
		<?php
		}
		...
		?>
			<div class="feature-section portfolio-kit-section portfolio-kit-section-support three-col">
		...
		public function free_pro()
		{
			...
			if (!empty($free_pro)) {
				if (!empty($free_pro['free_theme_name']) && !empty($free_pro['pro_theme_name']) && !empty($free_pro['features']) && is_array($free_pro['features'])) {
					...
					echo '<tr><td style="border-top:0;"></td><td style="border-top:0;"></td><td style="border-top:0;" class="only-pro"><a href="' . esc_url('https://wpthemespace.com/product/portfolio-kit-pro/') . '" target="_blank" >';
					echo esc_html('Portfolio Kit Pro Live Preview', 'portfolio-kit');
					echo '</a></td></tr>';
			...
			if ('appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix) {
				...
				wp_localize_script(
					...
					array(
						...
						'activating_string'        => __('Activating', 'portfolio-kit')
					)
				);
			}
		}
